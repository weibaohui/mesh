...
func Register(ctx context.Context, rContext *types.Context) error {
	rContext.Mesh.SetThreadiness(v1.SchemeGroupVersion.WithKind("Feature"), 1)

	f := &featureHandler{
		ctx:            ctx,
		namespace:      rContext.Namespace,
		featuresClient: rContext.Mesh.Mesh().V1().Feature(),
		featuresCache:  rContext.Mesh.Mesh().V1().Feature().Cache(),
		featureState:   map[string]func(){},
		apply: rContext.Apply.
			WithSetID("features").
			WithStrictCaching().
			WithCacheTypes(rContext.Mesh.Mesh().V1().Feature()),
	}
